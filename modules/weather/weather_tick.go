...
func (m *weatherModule) loadContent() {
	...
	seasonalTables, err := content.LoadSeasonalEmotes(dataFS, "weather/emotes/seasons")
	if err != nil {
...
// loadSeasons loads season tracks and establishes the baseline per-zone season
// map. Fail-soft ladder (design spec §7): disabled by config, no usable
// calendar, or no/invalid track files => m.seasonsOn stays false and weather
// runs exactly as v1. Loads from DOGMud's os.DirFS datafiles tree.
func (m *weatherModule) loadSeasons() {
	...
	dataFS := os.DirFS(configs.GetFilePathsConfig().DataFiles.String())
	tracks, errs := seasons.Load(dataFS, "weather/seasons", months, days)
	...
	m.tracks = tracks
	m.seasonsOn = true
	// Baseline resolution establishes zoneSeasons WITHOUT emitting events, so
	// reboots never replay a flood of season changes.
...
// WeatherSeasonChanged event for each calendar flip since the previous tick.
// Cross-track changes (a zone's biome reassigned by a rebuild) are not flips
// and emit nothing — listeners may assume From/To share a track.
...
func (m *weatherModule) tick(round uint64) {
	climate := m.climate
	if m.seasonsOn {
		// Season-adjusted odds: a pure transform on the climate table; sim.Step
