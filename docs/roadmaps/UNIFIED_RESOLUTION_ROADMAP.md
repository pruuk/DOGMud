...
| **U6b** | 🔴 **FINISH THE FLIP. Added 2026-08-19; the largest outstanding defect in the arc.** U6 deleted the legacy per-channel parameters for melee and taunt and **left spell, ranged and the special-move family on their old semantics**, which is the half-converted failure the sequencing section explicitly rejected. Verified: the spell channel still runs **two** contests, a binary hit gate scoring attacker `Wil + spellcasting x SkillWeight x SpellAttackSkillFactor` (skill **x15**) against the defender's **raw Willpower** (skill **x0**), with quell bolted on afterwards as damage-only mitigation that is **skipped entirely on a crit**. Ranged never reaches `ResolveChannelDefence` at all (`rangedDefenseScore`), and `ExecuteSkillMove`'s 14 callers add skill raw at x1/x1. **Two of this roadmap's own "Done when" criteria are therefore false today**: defence skill weight is not x5 in every channel, and `SpellAttackSkillFactor` is not gone from the attack path. 🔑 **And a THIRD axis found 2026-08-19 when the owner asked "what about crit?": `combat.ExecuteSkillMove` computes NO CRIT AT ALL** (`SkillMoveResult` has no `Crit` field), so **16 attacks in the game cannot critically hit** -- every special move plus every aimed shot. The **counterattack tier is melee-only** too (`ParryCritDetected`/`DodgeCritDetected`/`BlockCritDetected` are set in one place), so four of five channels have the defensive-crit curve without the tier the parent spec says a defensive crit IS. Scope: one contest per action through `combat.RunContest` with a channel-appropriate defence set, crit from that one margin everywhere, damage from that same margin, uniform x5, `SpellAttackSkillFactor` / `spellDefenseValue` / `rangedDefenseScore` / `RangedShieldDefenseBonus` / the `if !isCrit` defence skip all deleted, and **the counterattack tier extended to every channel gated on REACH** (melee counter when same-room; the cross-room `shoot` is the one coherent exception -- you cannot punch someone in the next room). **Mandatory modelling gate BEFORE any code**: this compounds x1→x5 with a brand-new crit tier on the same 16 attacks, and every mob carries combat skill 1. Spec: [`2026-08-19-u6b-finish-the-flip-design`](../superpowers/specs/2026-08-19-u6b-finish-the-flip-design.md). **Must land before U11.** | L | U9 | **Yes** |
...
| **U10c** | **Charm redesign.** Added 2026-08-19. `charm_spell.go` scores `Charisma + Manifestation x 25` — a skill weight of **25** against the intended uniform 5.0, so it survived the flip. It compensated for a periodic resist ladder that is **dead code**: nothing assigns `CharmDuration` at cast time and the ladder is gated on it being non-zero, so a charmed mob is charmed for 99999 rounds with no resist check ever. Give charm a real duration, delete the ladder, return the weight to 5.0. **Also decide charm's DEFENCE stat** — it is the only manifestation spell that attacks anyone, it resists on Willpower today, and Charisma is equally defensible for an act of social domination whose attack side is already Charisma. No migration needed (owner: no veteran uses charm). | M | U9 | **Yes** |
...
2. Defence skill weight is ×5 in every channel; `SpellAttackSkillFactor` is gone
   ...
   > 🔴 **FALSE as of 2026-08-19, and it has been false since U6 was declared
   > done.** The spell channel's hit gate scores the defender's **raw
   > Willpower** with no skill term at all (×0), and `SpellAttackSkillFactor`
   > ships at **3** and is still read by `characters.CalcSpellAttack`. Ranged
   > and `ExecuteSkillMove`'s 14 callers are also unconverted. **U6b** owns
   > finishing it.
   >
   > This criterion is prose, so nothing failed when it stopped being true.
   > U11 must express it as a test — see the U11 row.
