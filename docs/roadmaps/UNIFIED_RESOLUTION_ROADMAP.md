...
| **U10d** | **Surprise-attack redesign.** Split from U10 2026-08-21 (owner). The primary swing is an unconditional auto-hit with no defender term; redesign requires brainstorm → spec → plan (decided 2026-08-13). Sequenced with U10b/U10c before U12. **Spec written 2026-08-25:** [`2026-08-25-u10d-surprise-attack-redesign-design`](../superpowers/specs/2026-08-25-u10d-surprise-attack-redesign-design.md). Scope grew on investigation: **three** overlapping mechanics plus a latent bug, not one defect. `OnCombatRoundEnd` is called only from a test, so the surprise-round boundary does not exist in production and must be built. | M | U1, U0 | **Yes** |
