...
| **U12** | **Targeting and target-switching audit.** Added 2026-08-14 at the owner's request. This code has been rewritten several times and never re-examined as a whole; the arc has since removed a great deal of the complexity it was written around. Re-read it end to end and simplify what the flip made redundant. Surface: `actions/target_resolution.go`, `target_helpers.go`, `melee_target.go`, `sleeping_target.go`, `usercommands/target.go`, and `internal/parser` (~780 lines together). Audit first, then propose — the deliverable is a findings pass, and any simplification is scoped from what it finds rather than assumed up front. **SUPERSEDED 2026-08-29 by the approved design, [`2026-08-29-u12-unified-targeting-design`](../superpowers/specs/2026-08-29-u12-unified-targeting-design.md).** The audit was done and found the surface is **1090** non-test lines (not ~780), that targeting has **three** stores (`Aggro`, `CombatPhase`, `EvalContext.SoftTarget`) with a convention-only dual-write, and that the write surface is **90 sites** (47 `SetAggro` + 43 `EndAggro`, the latter missed on first count). At the owner's direction the slice became a **unification**, not a simplification: one `internal/targeting` package owning `Select` / `Commit` / `Release` / `EngagementOf` / `ConsumeOpeningStrike`, which mob behaviors call instead of carrying their own target-picking. Ships as **THREE** slices, deliberately ordered design-first: **U12a** (S) builds the package and proves it on **four call sites only** — the three behaviortree target actions plus `StageMeleeTarget` — so the API is reviewed on its merits rather than at the bottom of a 90-site diff, and mob behaviors are fully on the seam at the end of the first slice; **U12b** (L) is the mechanical sweep of the remaining ~86 write sites, guarded by an AST test, ending with `SetAggro`/`EndAggro` deleted; **U12c** (L) collapses onto `combatphase`, deletes `Aggro`/`AggroType`/`SpellAggroInfo`/`combat_state_compat.go`, migrates the ~290 reads, and owns the adversarial playtest. The rejected two-slice split, and the rejected mechanical-first ordering (which would re-touch all 90 sites when `Select`'s design moved `Commit`'s shape), are both recorded in the spec's section 3. U12c finishes the standing `// TODO Task 18: remove legacy Aggro.Type fallback once Aggro is gone.` at `NewRound_DoCombat_helpers.go:904`. | **L** | U6 | **U12a/U12b No; U12c YES** (the row's original "No" no longer holds) |
