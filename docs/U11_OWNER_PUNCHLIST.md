...
| D2 | ✅ **DONE — four skullduggery knobs now SET** (`f32d38b48`): `StealCooldown` 60, `StealHiddenBonus` 25, `ShadowCooldown` 5, `SneakFailCooldown` 3. | They were absent, and absence meant ZERO, not "default". **Now a live balance change to feel:** steal is no longer spammable and hidden theft finally pays. Unproven in play — nobody has ever run with them on. `PackScatterRounds` (2) is still unset; it is mob pack behaviour, not skullduggery. |
