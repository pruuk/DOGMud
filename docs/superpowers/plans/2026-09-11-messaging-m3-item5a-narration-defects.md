...
- **The room_text rule moved from a questengine boot check into `quests.Quest.Validate`** (found by review in execution, commit `75e47a1f1`). Tasks 5 and 6 below describe `questengine.ValidateAllRoomText` wired into `LoadDataFiles`; the admin web quest editor never ran it, so a bad line saved to disk, the reindex panicked in a recovered listener with no reply, and the next boot failed. It also missed room_text nested in a sequence's `on_complete`. `quests.RoomTextProblems`, checked from `Validate`, now refuses at save and still fails the boot; the questengine check is removed.
- **Lane C must not use a light buff for its dark checks** (found in the first lane C run, 2026-09-11). Chrysalis Glow's buff, Illumination, is a light source: its start line correctly reaches an unsighted witness because it is sent after the light exists, and it lights the cave for 20 minutes, which spoiled every later dark step. The committed lane uses Conviction Armor for the dark checks and casts Glow last, after the draught. The YAML blocks in Task 8 below show the first draft; the committed files are authoritative.
