...
- **The reservation ceiling** (cap total reservation at 50-75% of a pool, refuse the breaching wield/enchant/summon). Arc-scoped, slice to be decided, and it must precede U8.
  - **Companions are in scope** (owner, 2026-08-15): clamp their reservation too, and update the mob equip-if-better path so a companion that cannot wear a gift declines gracefully. `Character.Wear` already returns a `failureReason`; `internal/rooms/rooms.go:887` and `internal/bountyhunter/bountyhunter.go:91` check it, but `internal/actions/sell.go:339` discards it and would fail silently.
  - **Companions DO reserve on prod today**, visible in their resource bars. `GetPoolReservation` (`internal/characters/validate.go:245`) has no `IsMob` gate. This falsifies the premise behind five raw-max sweep sites rejected during U7 review ("mobs cannot carry a reservation"): three self-side reads in `internal/combat/ai.go`, plus `conditions_mob.go`, `action_cast_best_in_category.go` and `hpPercent` in `actions_archer.go`. A reserved companion currently reads as permanently wounded to those scorers, which is the same defect U7 fixed player-side. Re-examine them with this slice.
