...
| Retarget notice exists twice, both direct `SendText`, both name the target | `internal/hooks/NewRound_DoCombat.go:137,139` and `emitRetargetMessage`, `NewRound_DoCombat_unified.go:999-1016` |
...
3. **Retarget names the unseen.** `You turn your attention to X!` prints the new
   target's name regardless of the reader's sight.
...
**One retarget notice builder,** `retargetNotice(room, userId, target)`, used by
both call sites, hiding the name by the reader's `ParticipantSight`. The notice
