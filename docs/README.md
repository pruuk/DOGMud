...
| [`superpowers/audits/2026-09-07-narration-viewpoint-audit.md`](superpowers/audits/2026-09-07-narration-viewpoint-audit.md) | M1 of the messaging arc: all 247 hand-rolled narration sites ruled for actor/actee/observer coverage. 240 are correctly incomplete (the actee is a mob, the target is a door, the line is a refusal); 7 are defects. Every defect is the same shape — a duplicated code path that copied the effect and dropped a viewpoint — which is the argument for M2's central seam. Records that the scanner's labels are wrong 18% of the time and must not be read as findings |
