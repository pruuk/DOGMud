...
func TestThrowCostRefusalPreservesExactItemAndCombatState(t *testing.T) {
	...
	assert.False(t, target.Character.IsInCombat(), "refusal must not seed target aggression")
...
func TestThrowCostPaidAdmissionStaleTargetDoesNotResolveReplacement(t *testing.T) {
	...
	assert.False(t, originalTarget.Character.IsInCombat())
	assert.Equal(t, 500, replacement.Character.Health)
	assert.False(t, replacement.Character.IsInCombat(), "same-ID replacement must not be resolved")
