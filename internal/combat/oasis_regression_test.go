...
func TestOasisCritRatesAfterCompression(t *testing.T) {
	...
	// The shipped ContestGapSaturation. 0 is OFF.
	const shipped = 2.80

	critRate := func(def, k float64) float64 {
	...
	stormNow := critRate(86, 0)
	stormAfter := critRate(86, shipped)
	...
	royalNow := critRate(276, 0)
	royalAfter := critRate(276, shipped)
	...
	assert.Less(t, critRate(168, shipped), 0.50,
	...
	assert.InDelta(t, critRate(455, 0), critRate(455, shipped), 0.02,
		"an even fight must be untouched at any saturation")
}
